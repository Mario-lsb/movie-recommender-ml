...
class UserBasedRecommender:
    ...
    def recommend(self, user_id: int, n: int = 10):
        ...
        sim_users = self.user_similarity[user_id].drop(index=user_id) # type: ignore
        top_k_users = sim_users.nlargest(self.k)

        scores = {}
        for item_id in unrated_items:
            item_col = self.user_item_matrix[item_id] # type: ignore
            # Only users who rated this item
            rated_mask = item_col[top_k_users.index] > 0
            relevant_users = top_k_users[rated_mask]
            if relevant_users.sum() == 0:
                continue
            scores[item_id] = np.dot(
                relevant_users,
                item_col[relevant_users.index]
            ) / (relevant_users.sum() + 1e-8)
